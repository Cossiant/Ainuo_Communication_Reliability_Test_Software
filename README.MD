...
[![Version](https://img.shields.io/badge/version-v3.4.7-blue)](https://github.com)
...
> 💡 **设置命令（无回复）:** 对于 `*RST`、`CONF:VOLT:DC 10` 等电源不回复的设置命令，只需将 B 列留空，软件会自动跳过等待回复，仅按 C 列延时后发送下一条。查询命令（如 `*IDN?`）在 B 列填写期望值即可自动校验。

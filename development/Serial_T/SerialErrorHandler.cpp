...
SerialErrorHandler::SerialErrorHandler(SerialPage* page)
    : QObject(page), m_page(page)
{
}

QString SerialErrorHandler::bytesToDisplayText(const QByteArray &data, bool isHexMode)
{
    if (data.isEmpty())
...
void SerialErrorHandler::addTimeoutError(const QString &command, const QByteArray &expected)
{
    ++m_page->m_errorSeq;
    ...
    bool hexMode = m_page->m_serialHexSendCheckBox && m_page->m_serialHexSendCheckBox->isChecked();
...
void SerialErrorHandler::addContentError(const QString &command,
...
void SerialErrorHandler::clearErrors()
{
    m_page->m_errorSeq     = 0;

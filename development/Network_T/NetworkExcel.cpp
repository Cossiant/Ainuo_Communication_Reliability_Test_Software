...
// ═══════════════════════════════ 区间判断（使用公共工具）════════
bool NetworkExcel::tryRangeCompare(const QByteArray &received,
...
QByteArray NetworkExcel::stickyDelimiter() const {
    ...
    return StickySplitter::delimiterFromComboText(
        m_page->m_networkSplitDelimiterComboBox->currentText());
}

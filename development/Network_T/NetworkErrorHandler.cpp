...
NetworkErrorHandler::NetworkErrorHandler(NetworkPage* page)
    : QObject(page), m_page(page)
{
}

QString NetworkErrorHandler::bytesToDisplayText(const QByteArray &data, bool isHexMode)
{
    if (data.isEmpty())
...
void NetworkErrorHandler::addTimeoutError(const QString &command, const QByteArray &expected)
{
    ++m_page->m_errorSeq;
    ...
    bool hexMode = m_page->m_networkHexSendCheckBox && m_page->m_networkHexSendCheckBox->isChecked();
...
void NetworkErrorHandler::addContentError(const QString &command,
...
void NetworkErrorHandler::clearErrors()
{
    m_page->m_errorSeq     = 0;
